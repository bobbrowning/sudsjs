...
function connect() {
  ...
  try {
    let authString='';
    if (auth.couch) {authString=`${auth.couch.user}:${auth.couch.password}@`;}
    let url=`http://${authString}${suds.couch.connection.host}`;
    trace.log(url);
    const opts = {
      url: url,
       requestDefaults: suds.couch.connection.requestDefaults,
    }
    nano = Nano(opts);
 
    globalThis.db = nano.db.use(suds.couch.connection.database);
    console.log(`Connected to Couchdb database server`);
  ...
  nano.db.list().then(
    function (list) {
      trace.log({list: list})
      if (list.includes(suds.couch.connection.database)) {
        console.log(`Database ${suds.couch.connection.database} alive and well`)
      }
      else
      {
        console.log(`Couch system doesn't include ${suds.couch.connection.database}`,trace.line('s'))
...
function rawAttributes(table) {
  ...
  trace.log(Object.keys(combined))
  return (combined);
}

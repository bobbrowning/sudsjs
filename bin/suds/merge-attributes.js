...
module.exports = function (table, permission, subschemas, additionalAttributes) {
  ...
  trace.log({ cachekey: cachekey, cached: Object.keys(cache) })
  ...
  if (!cache[cachekey]) {
    ...
    trace.log({ creating: 'cache for ', table: table, cachekey: cachekey })
     ...
     * */
    let groupLookup = invertGroups(tableData, rawAttributesDeep);
    trace.log(groupLookup, { level: 'verbose' });

    /** Standardise attributes */
    let standardisedAttributes = standardise(table, rawAttributesDeep, groupLookup, [], [], permission);
    ...
    */
    if (subschemas && tableData.list && tableData.list.subschemaName) {
      for (let key of Object.keys(additionalAttributes)) {
...
function standardise(
  ...
  trace.log({ arguments: arguments, level: 'verbose' });
